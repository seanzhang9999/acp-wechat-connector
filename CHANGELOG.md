# v0.17.0 — WorkBuddy 会话控制权转移与双入口说明

- WorkBuddy 提供 owner／standby 模式；锁被占用时待命进程仍暴露 MCP 工具，不初始化业务资源。
- 新增 awei_transfer 与 awei_acquire：生成 6 位、10 分钟有效转移码，正常释放旧持有者后竞争取得唯一写权限；忙碌拒绝释放。
- 空闲时普通启动或消息提交可领取写权限，MCP 领取路径跳过旧交接宽限；无默认强制杀进程。
- 保留 v0.16 管理上下文自动轮换，更新扫码直连与 WorkBuddy 两种用法、Skill、安装与交接指南。
- 明确两入口目前是独立实例；共享阿维服务和 awiki 客户端接入属于未来架构，未宣称已实现。
- 用户于 2026-09-20 反馈 WorkBuddy 会话交接测试完成；此反馈不等于多入口并发和所有异常接管均已验收。

# v0.16.0 — 阿维管理上下文自动轮换

- iLink 与 WorkBuddy 共用的 ACP 语言服务接收 usage_update，默认用量达到 75% 后在下一条请求前轮换。
- 备用上限为每个管理会话 80 次模型调用或累计 240000 字符；不把字符估算显示为 token 用量。
- 保留控制器和路由状态，注入最近三轮有长度限制的交流记录，确认不重放、不延期，业务会话不重建。
- 不在同一请求的多步查阅中轮换；内部新会话继续过滤，旧会话关闭连接但不删除历史。
- “阿维，上下文状态”可直接查看最近用量、阈值与轮换次数，不调用模型。
- 新增用量、备用上限、压缩后用量下降、忙碌边界、查阅游标与确认流程测试。
