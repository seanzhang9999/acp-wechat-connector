...
async function main() {
  ...
  const server = new McpServer({ name: "awei-dedicated-session", version: "0.14.0" });
  ...
  server.registerTool("awei_status", { description: "检查专用阿维连接器及最近回执，不读取 WorkBuddy 会话。", inputSchema: {} }, async () => result({ version: "0.14.0", ...jobs.status() }));
  let closing = false;
