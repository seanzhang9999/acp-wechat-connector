...
文字和已有语音转写都识别消息开头的“阿维”及常见同音写法，例如“阿伟、阿唯、阿威、啊维、A维、a微、Ａ薇”；名字后可以省略标点或空格。只放宽开头称呼，不扫描正文。确认短句允许末尾句号、问号或感叹号，仍要求两分钟内匹配待确认动作。“潘伟”等首字也被错转的人名不自动唤醒，可重发“A维，…”或使用精确命令。纯语音业务消息的转写直接进入当前会话；没有转写时提示转文字。实际可用性取决于微信是否提供转写字段。
...
| `/acp codex start`（或 `open`） | 启动桌面，不释放业务会话 |
| `/acp codex restore` | 释放微信业务会话后启动桌面，保持阿维服务 |
...
- `/acp codex restore`：先按现有忙碌检查释放桥接业务会话，再启动桌面，保留阿维服务。任务、审批或传输未结束时拒绝交接；若启动失败，明确告知已释放的状态。
